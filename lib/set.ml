...
module AVL (T : Comparable) : sig
  ...
  let rot_right t =
    match t with
    | b, Node ((bl, Node (ll, vl, lr)), v, r) ->
        (max (bl + 1) b, Node (ll, vl, (b + 1 + min 0 bl, Node (lr, v, r))))
    | _ -> raise (BugRotate t)

  let rot_left t =
    match t with
    | b, Node (l, v, (br, Node (rl, vr, rr))) ->
        (min (br - 1) b, Node ((b - 1 - max 0 br, Node (l, v, rl)), vr, rr))
    | _ -> raise (BugRotate t)
...
let%expect_test "avlExpect" =
  let avl = List.range ~stride:(-1) 1000 0 in
  let avl =
